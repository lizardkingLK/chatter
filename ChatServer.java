...
public class ChatServer {
    ...
    private static HashMap<String, PrintWriter> namesMap = new HashMap<>();
    ...
    private static class Handler implements Runnable {
        ...
        public void run() {
            try {
                ...
                namesMap.put(name, out);
            	for (String key : namesMap.keySet()) {
            		if(!key.equals(name))
            			namesMap.get(key).println("NAME"+name);
            	}
                ...
                while (true) {
                    ...
                    if(input.startsWith("MULTICAST")) {
                		...
                		String msg = namesArr[0];
                		String sel = namesArr[1];
                		
                		String selected = sel.substring(1, sel.length()-1);
                		
                		String[] selArr = selected.split(",");
                		
                		for (String selItem : selArr) {
                			String k = selItem.trim();
                			System.out.println(k);
                			if(namesMap.containsKey(k)) {
                				namesMap.get(k).println("MULTI " + name + ": " + msg);
                			}
						}
                		namesMap.get(name).println("MULTI " + name + ": " + msg);
                    }
                    else {
                ...
                if (name != null) {
                    for (String key : namesMap.keySet()) {
                		if(!key.equals(name)) {
                			namesMap.get(key).println("EXIT"+name);
                		}
                	}
